...
partA earliest departures =
  let afters = [(d, d - earliest `mod` d) | d <- departures]
      ds = L.sortOn (\(d, w) -> w) afters
  in  head ds


day13 ls =
  let (earliest, d) = parse ls
      departures = catMaybes d
      (depart, wait) = partA earliest departures
  in depart * wait
...
partB ds =
  let d' = [0..] `zip` ds
         & filter (\(f, m) -> isJust m)
         & map (\(f, Just m) -> (m, -f))
  -- the equations are t = mx - f
      (m, f) = foldl1 combineDiophantine d'
      l = d' & map (\(m, f) -> m) & foldl1 lcm
  -- we want the smallest t > 0
  -- t - f = mx  - m will be +/- lcm
