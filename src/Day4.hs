...
parsePassport s = quickParse lineParser s
...
byrValid p =
  case (byr p) of
    Nothing -> False
    Just y -> case quickParse natParser y of
                Nothing -> False
                Just year -> 1920 <= year && year <= 2002
...
hgtValid p =
  case (hgt p) of
    Nothing -> False
    Just h -> case quickParse (natParser <<>> munch1 isAlpha) h of
                Nothing -> False
                Just (h, "cm") -> 150 <= h && h <= 193
                Just (h, "in") -> 59 <= h && h <= 76
                _ -> False
...
hclValid p =
  case (hcl p) of
    Nothing -> False
    Just h -> case quickParse (char '#' >>>> count 6 (satisfy isHexDigit)) h of
                Nothing -> False
                _ -> True
...
eclValid p =
  case (ecl p) of
    Nothing -> False
    Just c -> c == "amb" || c == "blu" || c == "brn" || c == "gry" || c == "grn" || c == "hzl" || c == "oth"

--    pid (Passport ID) - a nine-digit number, including leading zeroes.
pidValid p =
  case (pid p) of
    Nothing -> False
    Just pid -> case quickParse (count 9 (satisfy isDigit)) pid of
                Nothing -> False
                _ -> True
...
day4b ls =
  let passports = parse ls
  in filter isMoreValid passports & length