...
numbers input =
  let prefix = take (length input - 1) input
      ...
      ns = prefix ++ continue (length input & fromIntegral) (last input) lastSpoken
  in ns
  where
    continue :: Integer -> Integer -> Map.Map Integer Integer -> [Integer]
    continue thisTurn aboutToSay lastSpoken =
